...
if __name__ == '__main__':
    # 测试四则运算的广播功能
    x = Variable(np.array([3, 3, 3]))
    y = Variable(np.array([[6, 6, 6], [9, 9, 9]]))
    z = x + y
    z.backward()
    print("x.grad:", x.grad)  # variable([2 2 2])
    print("y.grad:", y.grad)  # variable([[1 1 1] [1 1 1]])

    x.grad, y.grad, z.grad = 0, 0, 0
    z = x * y
    z.backward()
    print("x.grad:", x.grad)  # variable([15 15 15])
    print("y.grad:", y.grad)  # variable([[3 3 3] [3 3 3]])
    ...
    z.backward()
    print("x.grad:", x.grad)  # variable([-2 -2 -2])
    print("y.grad:", y.grad)  # variable([[1/3 1/3 1/3] [1/3 1/3 1/3]])
